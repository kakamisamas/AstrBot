...
async def test_reasoning_only_response_materializes_visible_fallback(
    ...
    assert llm_results[0].data["chain"].type == "reasoning"
    assert llm_results[0].data["chain"].get_plain_text() == "step 1"
    assert llm_results[1].data["chain"].get_plain_text() == final_resp.completion_text
